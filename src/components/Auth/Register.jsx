...
const Register = () => {
  ...
  const signUpUser = async (userInfo) => {
    ...
    try {
    const {data} = await axios.post(isLogin ? (`${process.env.REACT_APP_API_URL}/api/auth/login`) : (`${process.env.REACT_APP_API_URL}/api/auth/signup`),userInfo).then((response) => {
        //console.log(response);
        const userData = { token: response.data.token, userInfo: response.data.data.user };
        localStorage.setItem("user", JSON.stringify(userData));
        message.success(`${isLogin ? "Sign In " : "Sign up"} successful...`, 2);
      ...
      >
        <div className="auth">
          ...
          <Form onFinish={onFinish} layout="vertical">
            ...
            >
              <Input placeholder={isLogin ? "Try (admin@gmail.com)" : "Email Address"} />
            </Form.Item>
            <Form.Item
              name="password"
              rules={[
