...
const BookingRoomModal = ({
      ...
      try {
        await Promise.all(
          selectedRooms?.map((roomId) => {
            const { res } = axios.patch(`${process.env.REACT_APP_API_URL}/api/room/availability`,{roomId,dates: allDates}).then((response) => {
  ...
  const createNewBooking = async (bookingData) => {
   // console.log(bookingData);
    try {
      const data = await axios.post(`${process.env.REACT_APP_API_URL}/api/booking/addBooking`,bookingData).then((response) => {
        //console.log(response);
        if (response) {
          swal(
